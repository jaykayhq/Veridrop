    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] text-[#e8e8e8] font-sans flex items-center justify-center">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-[#666]">Loading tracking...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] text-[#e8e8e8] font-sans flex items-center justify-center">
        <div className="text-center max-w-sm mx-6">
          ...
          <p className="text-sm text-[#666] mb-6">This transaction does not exist or has been removed.</p>
          <Link href="/" className="text-sm text-[#00bda6] hover:underline">Go Home</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-[#0a0a0a] text-[#e8e8e8] font-sans flex items-center justify-center">
        <div className="text-center max-w-sm mx-6">
          ...
          <button
            ...
            className="text-sm text-[#00bda6] hover:underline"
          >
            Try Again
          </button>
        </div>
      ...
      <div className="mx-auto max-w-4xl px-6 py-8 space-y-6">
        <div className="bg-[#111] rounded-xl border border-[#1a1a1a] p-6">
          <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
            <div>
              <p className="text-xs text-[#666] uppercase tracking-wider mb-1">Order</p>
              <h1 className="text-lg font-semibold font-mono">{order?._id}</h1>
              <p className="text-sm text-[#666] mt-1">{order?.productName}</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-[#555] mb-0.5">Buyer</p>
              <p className="text-[#e8e8e8]">{order?.buyerName}</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-wider text-[#555] mb-0.5">Vendor</p>
              <p className="text-[#e8e8e8]">{order?.vendorName}</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-wider text-[#555] mb-0.5">Amount</p>
              <p className="text-[#00bda6] font-semibold">
                {order && formatCurrency(order.amount)}
              </p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-wider text-[#555] mb-0.5">Escrow</p>
              <p className="text-[#e8e8e8] capitalize">{order?.escrow?.status || "N/A"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#111] rounded-xl border border-[#1a1a1a] p-6">
          ...
          <div className="relative">
            <div className="absolute left-[11px] top-1 bottom-1 w-0.5 bg-[#1f1f1f]" />
                  ...
                  <div key={step} className="relative flex items-start gap-4">
                    <div
                      className={`relative z-10 mt-0.5 h-6 w-6 shrink-0 rounded-full border-2 flex items-center justify-center text-[10px] font-bold transition-colors ${
                        isCurrent
                          ? "border-[#00bda6] bg-[#00bda6] text-white"
                          : isActive
          ...
          <div className="bg-[#111] rounded-xl border border-[#1a1a1a] p-5">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="rounded-lg bg-gradient-to-r from-[#0a54a6] to-[#00bda6] px-5 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-90"
              >
                Approve Release
              </button>
              <button
                onClick={handleFileDispute}
                className="rounded-lg border border-[#333] px-5 py-2.5 text-sm text-[#aaa] transition-colors hover:border-red-500/50 hover:text-red-400"
              >
                File Dispute
              </button>
            </div>
